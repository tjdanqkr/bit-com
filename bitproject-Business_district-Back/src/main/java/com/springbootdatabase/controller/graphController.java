...
public class graphController {
    ...
    public HashMap<String, List<Jsonreturn>> json1(@RequestBody final HashMap<String, Object> post,
    ...
    public dining[] dining(@RequestBody final HashMap<String, Object> post, HttpServletRequest request)
            throws Exception {
    ...
    public instarDto[] instar(@RequestBody final HashMap<String, Object> post, HttpServletRequest request)
            throws Exception {
        request.setCharacterEncoding("UTF-8");
        RestTemplate restTemplate = new RestTemplate();
        HashMap<String, String> map = new HashMap<>();
        ...
        ResponseEntity<instarDto[]> responseEntity = restTemplate.exchange(url, HttpMethod.POST, entity,
                instarDto[].class);
    ...
    @ResponseBody // 통신 메시지 관련 header와 body의 값들을 하나의 객체로 저장하는 것이 HttpEntity 클래스 객체,Request 부분일 경우
                  // HttpEntity를 상속받은 RequestEntity가, Response 부분일 경우 HttpEntity를 상속받은
                  // ResponseEntity가 하게
    @RequestMapping(value = "/api/chui", method = RequestMethod.POST) // RestTemplate 이용하여 Flask에 접속
    public ChuiDto[] chui(@RequestBody final HashMap<String, Object> post, HttpServletRequest request)
            throws Exception {
        request.setCharacterEncoding("UTF-8");
        ...
        // map.put("classes",post.get("classes").toString());//request로 날아오는것에서 원하는 정보
        // 빼줌
    ...
    @ResponseBody
    @RequestMapping(value = "/api/instar2", method = RequestMethod.GET) // api/instar2가 키, 기능에 따라 method =
                                                                        // RequestMethod.?이 들어가는것.
    public List<String> instar2() throws Exception {
        ...
        HttpEntity<?> entity = new HttpEntity<>(map);
        ResponseEntity<instarDto2> responseEntity = restTemplate.exchange(url, HttpMethod.GET, entity,
                instarDto2.class);
        System.out.println(responseEntity.getBody());
    ...
    public ResponseEntity<instarDto[]> json2(@RequestBody final HashMap<String, Object> post,
            HttpServletRequest request) throws Exception {
        request.setCharacterEncoding("UTF-8");
        RestTemplate restTemplate = new RestTemplate();
        HashMap<String, String> map = new HashMap<>();
